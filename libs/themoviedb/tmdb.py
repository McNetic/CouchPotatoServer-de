...
def configure(api_key, lang='en', country='us'):
    ...
    config['urls']['imdb.lookUp'] = "http://api.themoviedb.org/2.1/Movie.imdbLookup/%(lang)s/xml/%(apikey)s/%%s" % (config)
    config['urls']['movie.browse'] = "http://api.themoviedb.org/2.1/Movie.browse/%(lang)s-%(country)s/xml/%(apikey)s?%%s" % (config)
    #raise Error(repr(config['urls']))
